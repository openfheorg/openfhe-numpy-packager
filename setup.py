...
filepath = os.path.join(
    os.path.dirname(os.path.abspath(__file__)), "ci-vars.sh"
)
...
get_version = (
    ...
    ci_vars["OS_RELEASE"],
    ci_vars["OPENFHE_NUMPY_TAG"],
    ci_vars["WHEEL_MINOR_VERSION"],
    ci_vars["WHEEL_TEST_VERSION"],
)
...
version = subprocess.run(
    get_version, shell=True, capture_output=True, text=True
).stdout.strip()
long_description = subprocess.run(
    get_long_descr, shell=True, capture_output=True, text=True
).stdout.strip()
setup(
    name="openfhe_numpy",
    version=version,
