# Use Ubuntu 24.04 as the base image and set the correct OS description in ci-vars.sh below
FROM ubuntu:24.04
...
RUN sed -i '/^OS_RELEASE=/c\OS_RELEASE=24.04' /root/openfhe-numpy-packager/ci-vars.sh
