...
#   echo "Or call via symlink named ubuntu20.sh / ubuntu22.sh / ubuntu24.sh ..." >&2
    exit 2
fi
OS_RELEASE="$1"
...
CONTAINER_NAME="openfhe-numpy-build:${OS_NAME}_${OS_RELEASE}"
...
# copy the wheel to the local machine
docker cp "${TMP_CONT}:/root/openfhe-numpy-packager/build/dist" "${LOCAL_DIR}"
